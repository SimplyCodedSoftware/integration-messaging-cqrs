...
use Fixture\Annotation\CommandHandler\Aggregate\AggregateCommandHandlerWithEnrichInterceptor;
...
class CqrsMessagingModuleTest extends TestCase
{
    ...
    public function test_registering_aggregate_command_handler_with_pre_call_interceptors()
    {
        $commandHandler = CqrsMessageHandlerBuilder::createAggregateCommandHandlerWith(DoStuffCommand::class, AggregateCommandHandlerWithPreCallInterceptorExample::class, "interceptedCommand")
            ...
            ->withPreCallInterceptors(
                [
                    ReferenceCallInterceptor::create(
                        "some", "action", [
                        MessageToPayloadParameterConverterBuilder::create("command")
                    ]
                    )
    ...
    public function test_registering_aggregate_command_handler_with_interceptors_and_exclusions()
    {
    ...
    public function test_registering_aggregate_command_handler_with_enrich_interceptor()
    {
        $commandHandler = CqrsMessageHandlerBuilder::createAggregateCommandHandlerWith(DoStuffCommand::class, AggregateCommandHandlerWithEnrichInterceptor::class, "interceptedCommand")
            ->withOutputMessageChannel("nullChannel")
            ->withMethodParameterConverters([MessageToPayloadParameterConverterBuilder::create("stuffCommand")])
            ->withPostCallInterceptors([
                EnrichCallInterceptor::create([
                    ExpressionPayloadSetterBuilder::createWith("token", "123"),
                    ExpressionHeaderSetterBuilder::createWith("password", "abc")
                ])  ->withRequestChannelName("requestChannel")
                    ->withRequestPayloadExpression("payload")
                    ->withRequestHeaders(["key" => "value"])
            ]);
        ...
        $this->createModuleAndAssertConfiguration(
            [
                AggregateCommandHandlerWithEnrichInterceptor::class
            ], $expectedConfiguration
        );
    }
}